...
fun getStyle(
    ...
    if (BuildConfig.MAPBOX_KEY.isEmpty() && BuildConfig.STADIA_KEY.isEmpty()) {
        if (styleOptions != null) {
            Log.w(TAG, "Custom Google Maps styling is not supported by the no-key OpenFreeMap fallback")
        }
        return Style.Builder().fromUri(OPEN_FREE_MAP_STYLE_URI)
    }
